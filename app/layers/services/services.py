...
def getAllImages(request, input=None):
    if input is None:
        json_collection = transport.getAllImages(request, input=None) # obtiene un listado de datos "crudos" desde la API, usando a transport.py.
    else:
        json_collection = transport.getAllImages(request, input)
    images = []
